...
export default function FinancePage() {
  ...
  const [branches, setBranches] = useState<{ _id: string; name: string; city?: string }[]>([]);
  ...
  async function load() {
    ...
    try {
      ...
      const [plRes, yearRes, staffRes, brRes] = await Promise.all([
        fetch(`/api/finance?mode=pl&from=${from}&to=${to}${bq}`).then((r) => r.json()),
        fetch(`/api/finance?mode=pl&from=${yearStart}&to=${to}${bq}`).then((r) => r.json()),
      ...
      if (brRes.ok) setBranches(brRes.branches || []);
    } catch {
      ...
      <header className="glass sticky top-0 z-40 border-b border-white/5">
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-wrap gap-3 items-center justify-between">
          ...
          <div className="flex gap-2 items-center flex-wrap">
            ...
            <select value={branchId} onChange={(e) => setBranchId(e.target.value)} className="px-2 py-1.5 rounded-lg bg-slate-900 border border-white/10 text-sm">
