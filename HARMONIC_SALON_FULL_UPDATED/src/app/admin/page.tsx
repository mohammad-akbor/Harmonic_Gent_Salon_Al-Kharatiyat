...
export default function AdminPage() {
  ...
  function startEditStaff(s: Staff) {
    ...
    setStaffForm({
      ...
      branchId: (s as any).branchId ? String((s as any).branchId) : "",
  ...
  async function deleteStaff(id: string) {
    if (!confirm("Permanently DELETE this staff? This cannot be undone.")) return;
    ...
    <div className="min-h-screen pb-16">
      <header className="glass sticky top-0 z-40 border-b border-white/5">
        <div className="max-w-5xl mx-auto px-4 py-3 flex justify-between items-center flex-wrap gap-2">
          ...
          <Link href="/admin/offers" className="px-3 py-1.5 rounded-lg bg-[#d4af37]/20 text-[#d4af37] text-sm">Offers</Link>
        </div>
      </header>
        ...
        {tab === "staff" && (
          <div className="space-y-6">
            <form onSubmit={createStaff} className="glass rounded-2xl p-5 grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                <label className="text-xs text-slate-400">Branch / Location</label>
                <select value={staffForm.branchId} onChange={(e) => setStaffForm({ ...staffForm, branchId: e.target.value })} className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-white/10">
              ...
              <div className="flex gap-2 items-end">
                ...
                {editStaffId && (
                  <button type="button" onClick={() => { setEditStaffId(null); setStaffForm({ name: "", department: "Barber", phone: "", email: "", password: "", status: "Active", servicePercent: "40", productPercent: "5", salaryType: "Commission", fixedSalary: "0" }); }} className="px-3 py-2 rounded-lg bg-white/10 text-sm">
                    Cancel
                  </button>
                )}
              </div>
