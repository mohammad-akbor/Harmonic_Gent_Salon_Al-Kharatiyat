      ...
      <header className="glass sticky top-0 z-50 border-b border-white/5">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <Link href="/" className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#d4af37] to-[#b8860b] flex items-center justify-center font-black text-[#0b1220] text-lg shadow-lg logo-3d">
              H
            </div>
            <span className="font-bold text-lg tracking-wide">HARMONIC SALON</span>
          </Link>
          <nav className="flex items-center gap-2 sm:gap-3 text-sm flex-wrap justify-end">
            <Link href="/#services" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
              Services
            </Link>
            <Link href="/#team" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
              Team
            </Link>
            <Link href="/store" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
              Store
            </Link>
            <Link href="/booking" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
              Book
            </Link>

            {session ? (
              <>
                {/* Admin only — salary / finance / media hidden from staff & customer */}
                {isAdmin && (
                  <>
                    <Link href="/admin" className="px-3 py-1.5 rounded-lg bg-[#d4af37]/20 text-[#d4af37] hover:bg-[#d4af37]/30 transition">
                      Admin
                    </Link>
                    <Link href="/finance" className="px-3 py-1.5 rounded-lg bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 transition">
                      Finance
                    </Link>
                    <Link href="/salary" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
                      Salary
                    </Link>
                  </>
                )}
                {(isStaff || isAdmin) && (
                  <>
                    <Link href="/dashboard" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
                      Dashboard
                    </Link>
                    <Link href="/attendance" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
                      Attendance
                    </Link>
                  </>
                )}
                {isAdmin && (
                  <>
                    <Link href="/inventory" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
                      Inventory
                    </Link>
                    <Link href="/customers" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
                      Customers
                    </Link>
                  </>
                )}
                {role === "customer" && (
                  <Link href="/my-bookings" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
                    My Bookings
                  </Link>
                )}
                <Link href="/settings" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
                  Settings
                </Link>
                <button
                  onClick={() => signOut({ callbackUrl: "/" })}
                  className="px-3 py-1.5 rounded-lg border border-white/20 hover:bg-white/10 transition"
                >
                  Logout
                </button>
              </>
              ...
              <>
                <Link href="/login" className="px-3 py-1.5 rounded-lg hover:bg-white/10 transition">
                  Login
                </Link>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {gallery.map((g) => {
                const isVid =
                  ...
                  g.type === "story";
                return (
                  <div key={g._id} className="card-3d rounded-2xl overflow-hidden aspect-[4/3] bg-slate-900 relative">
                    {isVid ? (
                      <video
                        src={g.url}
                        muted
