...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    ["name", "department", "phone", "email", "salaryType", "status"].forEach((k) => {
...
/** DELETE — set Inactive (Admin only) */
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await connectDB();
    const staff = await Staff.findByIdAndUpdate(params.id, { status: "Inactive" }, { new: true });
    if (!staff) return NextResponse.json({ ok: false, error: "Not found" }, { status: 404 });
    return NextResponse.json({ ok: true, staff });
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : "Error";
